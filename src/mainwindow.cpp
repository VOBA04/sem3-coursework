...
void MainWindow::change_image(
    cv::Mat cv_im) // изменение отображаемого изображения
{
...
void MainWindow::start_proc(
    QString &QPath) // открытие изображения и сохранение его в программу
{
  QString Qpath_from;
  if (QPath.isEmpty()) {
    Qpath_from = QFileDialog::getOpenFileName(
        this, QObject::tr("Open file"), "", QObject::tr("Images (*.png *.jpg)"),
        nullptr);
  ...
  if (!Qpath_from.isEmpty()) {
    ...
    *(image_info.image_in_proc) = Qpixmap;
    ...
    // Обновляем список последних файлов в пользовательской папке данных
    // приложения
...
void MainWindow::main_proc(int value) // обработка изображения
{
  ...
  case PROCESSES::BRIGHTNESS: {
    mythread->push(
        new Oper_brightness(value, QtOcv::image2Mat(image.toImage())));
    break;
  }
  ...
  case PROCESSES::TEMPERATURE: {
    mythread->push(
        new Oper_temperature(value, QtOcv::image2Mat(image.toImage())));
    break;
  }
...
void MainWindow::save_image() // сохранения изображения на компьютер
{
  ...
  QString filename;
  // Предлагаем сохранить в папку Изображения пользователя (или Домашнюю)
  QString pictures =
  ...
  filename = QFileDialog::getSaveFileName(this, QObject::tr("Save File"),
                                          pictures, filter, &selected_filter);
  if (!filename.isEmpty()) {
    prepare_image();
